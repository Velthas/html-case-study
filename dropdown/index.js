const dropdownGenerator = (function () {
  ...
  function createSubItems(number) {
    ...
    for(let i = 0; i < number; i++) {
      ...
      const itemContent = prompt(`What will sub menu item no.${i + 1} contain?`);
  ...
  function createMenuItems(number) {
    ...
    for (let i = 0; i < number; i++) {
      const menuTextContent = prompt(`What will menu item no.${i + 1} contain?`);
      ...
      const numberOfChildren = prompt(`How many sub items will menu ${menuTextContent} have?`);
  ...
  function createDropdown() {
    ...
    const numberOfMenus = prompt('How many navbar items do you want? (numeric value)');
